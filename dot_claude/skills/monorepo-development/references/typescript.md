# Monorepo Development — TypeScript

pnpm Workspaces conventions.
...
**Each app still validates only its own variables.** A shared root `.env` doesn't mean every package reads every variable — per-package `env.ts` schemas (see the `zod-env` skill) each declare exactly the subset they need. The root file is just where the values live.
